#!/usr/bin/env bash
#
# Try `install_udpjt.sh --help` for usage.
#
# (c) 2025 UDP-MOD JT
#
...
show_usage_and_exit() {
    echo
    echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - UDP-JT MOD server install script"
    echo
    echo -e "Usage:"
    echo
    ...
    echo -e "\t-l, --local <file>\tInstall specified UDP-JT MOD binary instead of download it."
    echo -e "\t--version <version>\tInstall specified version instead of the latest."
    echo
    echo -e "$(tbold)Remove UDP-JT$(treset)"
    echo -e "\t$0 --remove"
    echo
...
perform_install_manager_script() {
    local _manager_script="/usr/local/bin/udpjt_manager.sh"
    local _symlink_path="/usr/local/bin/udpjt"
    
    echo "Downloading manager script..."
    curl -o "$_manager_script" "https://raw.githubusercontent.com/JotchuaDevz/UDP-JT-MANAGER/refs/heads/main/udpjt_manager.sh"
...
perform_install() {
    ...
    if [[ -n "$_is_fresh_install" ]]; then
        echo
        echo -e "$(tbold)Congratulations! UDP-JT MOD has been successfully installed on your server.$(treset)"
        echo "Use 'udpjt' command to access the manager."

    else
        restart_running_services
        start_services
        echo
        echo -e "$(tbold)UDP-JT MOD has been successfully updated to $VERSION.$(treset)"
        echo
    fi
}
...
perform_remove() {
    ...
    echo
    echo -e "$(tbold)Congratulations! UDP-JT MOD has been successfully removed from your server.$(treset)"
    echo
